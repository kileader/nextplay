...
- **Personal game status:** `UserGame.status` is an optional, user-managed value: `BACKLOG`, `PLAYING`, `COMPLETED`, or `DROPPED`. Existing and newly imported games remain uncategorized by default. Steam playtime and last-played facts never infer or overwrite this status.
- **Next v1 behavior:** The primary recommendation surface is **Next**, a time-neutral “what fits now?” flow, not a nightly-only ritual. Its first implementation returns three playable-library picks, excludes completed/dropped games, and uses explicit time/energy commitment heuristics, play history, status, and cached IGDB quality. Reasons must only claim data the picker actually used; genre/mood matching and AI are deferred.
- **IGDB descriptions:** Store IGDB `summary` in `game_cache.igdb_summary` during cache sync. It is optional enrichment, like covers: the Next UI must remain functional for unmatched Steam rows and state when a description is unavailable.
...
- **Steam import behavior:** A CSV import validates the whole file before saving, then treats it as the authenticated user's current Steam-library snapshot: it creates or updates rows by `(user_id, steam_app_id)` and removes that user's rows absent from the export. Re-importing a game preserves its user-managed status; revisit snapshot deletion only if a future source adds independently managed games.
...
- **IGDB Steam id:** The broad catalog sync resolves Steam IDs from the first Steam row in **`external_games`** (`category` 1, parse `uid`). Targeted library enrichment queries IGDB's external-game-source mapping first, then retains the requested AppID explicitly while caching the returned game. Do not request the unsupported `external.steam` game field. Numeric min/max across multiple Steam `uid`s was rejected — use the first matching row after filter.
